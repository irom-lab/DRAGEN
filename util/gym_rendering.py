...
class Image(Geom):
    def __init__(self, fname, width, height, loc_x=None, loc_y=None, alpha=1, fromarray=False, array_dim=64):
        Geom.__init__(self)
        self.set_color(1.0, 1.0, 1.0, alpha)
        self.width = width
        self.height = height
        if fromarray:
            img = pyglet.image.ImageData(array_dim, array_dim, 'RGB', fname, pitch=array_dim*3*1)
        else:
        ...
        if loc_x is not None:
            ...
            self.loc_x = -self.width/2
        if loc_y is not None:
            self.loc_y = loc_y
        else:
            self.loc_y = -self.height/2

    def render1(self):
        self.img.blit(self.loc_x, self.loc_y, width=self.width, height=self.height)
